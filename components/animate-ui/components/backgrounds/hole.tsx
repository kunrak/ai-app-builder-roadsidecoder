...
const DEFAULT_PARTICLE_COLOR: [number, number, number] = [255, 255, 255];

const HoleBackground = React.memo(function HoleBackground({
  strokeColor = "#737373",
  numberOfLines = 50,
  numberOfDiscs = 50,
  particleRGBColor = DEFAULT_PARTICLE_COLOR,
  className,
  children,
  ...props
}: HoleBackgroundProps) {
  ...
  const particleColorKey = particleRGBColor.join(",");
  ...
  const initParticle = React.useCallback(
    (start: boolean = false) => {
      ...
      const rgb = particleColorKey.split(",").map(Number);
      return {
        x: sx,
        sx,
        dx,
        ...
        p: 0,
        r,
        c: `rgba(${rgb[0]}, ${rgb[1]}, ${rgb[2]}, ${Math.random()})`,
      };
    },
    [particleColorKey],
  );
